...
(function(global) {
    ...
    fabric.util = {
        ...
        normalizePoints: function(points, options) {
            var minX = fabric.util.array.min(points, "x"), minY = fabric.util.array.min(points, "y");
            minX = minX < 0 ? minX : 0;
            minY = minX < 0 ? minY : 0;
...
(function() {
    ...
    function arcToSegments(x, y, rx, ry, large, sweep, rotateX, ox, oy) {
        ...
        var coords = getXYCoords(rotateX, rx, ry, ox, oy, x, y), d = (coords.x1 - coords.x0) * (coords.x1 - coords.x0) + (coords.y1 - coords.y0) * (coords.y1 - coords.y0), sfactorSq = 1 / d - .25;
        if (sfactorSq < 0) {
            sfactorSq = 0;
        }
        ...
        var xc = .5 * (coords.x0 + coords.x1) - sfactor * (coords.y1 - coords.y0), yc = .5 * (coords.y0 + coords.y1) + sfactor * (coords.x1 - coords.x0), th0 = Math.atan2(coords.y0 - yc, coords.x0 - xc), th1 = Math.atan2(coords.y1 - yc, coords.x1 - xc), thArc = th1 - th0;
        if (thArc < 0 && sweep === 1) {
            thArc += 2 * Math.PI;
        } else if (thArc > 0 && sweep === 0) {
            thArc -= 2 * Math.PI;
        }
        var segments = Math.ceil(Math.abs(thArc / (Math.PI * .5 + .001))), result = [];
        for (var i = 0; i < segments; i++) {
            var th2 = th0 + i * thArc / segments, th3 = th0 + (i + 1) * thArc / segments;
    ...
    function getXYCoords(rotateX, rx, ry, ox, oy, x, y) {
        var th = rotateX * (Math.PI / 180), sinTh = Math.sin(th), cosTh = Math.cos(th);
        rx = Math.abs(rx);
        ry = Math.abs(ry);
        var px = cosTh * (ox - x) * .5 + sinTh * (oy - y) * .5, py = cosTh * (oy - y) * .5 - sinTh * (ox - x) * .5, pl = px * px / (rx * rx) + py * py / (ry * ry);
        if (pl > 1) {
            pl = Math.sqrt(pl);
            rx *= pl;
            ry *= pl;
        }
        var a00 = cosTh / rx, a01 = sinTh / rx, a10 = -sinTh / ry, a11 = cosTh / ry;
        return {
            x0: a00 * ox + a01 * oy,
            y0: a10 * ox + a11 * oy,
            x1: a00 * x + a01 * y,
    ...
    function segmentToBezier(cx, cy, th0, th1, rx, ry, sinTh, cosTh) {
        ...
        var a00 = cosTh * rx, a01 = -sinTh * ry, a10 = sinTh * rx, a11 = cosTh * ry, thHalf = .5 * (th1 - th0), t = 8 / 3 * Math.sin(thHalf * .5) * Math.sin(thHalf * .5) / Math.sin(thHalf), x1 = cx + Math.cos(th0) - t * Math.sin(th0), y1 = cy + Math.sin(th0) + t * Math.cos(th0), x3 = cx + Math.cos(th1), y3 = cy + Math.sin(th1), x2 = x3 + t * Math.sin(th1), y2 = y3 - t * Math.cos(th1);
    ...
    fabric.util.drawArc = function(ctx, x, y, coords) {
        var rx = coords[0], ry = coords[1], rot = coords[2], large = coords[3], sweep = coords[4], ex = coords[5], ey = coords[6], segs = arcToSegments(ex, ey, rx, ry, large, sweep, rot, x, y);
...
(function(global) {
    "use strict";
    var fabric = global.fabric || (global.fabric = {}), extend = fabric.util.object.extend, capitalize = fabric.util.string.capitalize, clone = fabric.util.object.clone, toFixed = fabric.util.toFixed, multiplyTransformMatrices = fabric.util.multiplyTransformMatrices, attributesMap = {
        cx: "left",
        x: "left",
        r: "radius",
        cy: "top",
    ...
    fabric.parseTransformAttribute = function() {
        ...
        var iMatrix = [ 1, 0, 0, 1, 0, 0 ], number = "(?:[-+]?\\d+(?:\\.\\d+)?(?:e[-+]?\\d+)?)", commaWsp = "(?:\\s+,?\\s*|,\\s*)", skewX = "(?:(skewX)\\s*\\(\\s*(" + number + ")\\s*\\))", skewY = "(?:(skewY)\\s*\\(\\s*(" + number + ")\\s*\\))", rotate = "(?:(rotate)\\s*\\(\\s*(" + number + ")(?:" + commaWsp + "(" + number + ")" + commaWsp + "(" + number + "))?\\s*\\))", scale = "(?:(scale)\\s*\\(\\s*(" + number + ")(?:" + commaWsp + "(" + number + "))?\\s*\\))", translate = "(?:(translate)\\s*\\(\\s*(" + number + ")(?:" + commaWsp + "(" + number + "))?\\s*\\))", matrix = "(?:(matrix)\\s*\\(\\s*" + "(" + number + ")" + commaWsp + "(" + number + ")" + commaWsp + "(" + number + ")" + commaWsp + "(" + number + ")" + commaWsp + "(" + number + ")" + commaWsp + "(" + number + ")" + "\\s*\\))", transform = "(?:" + matrix + "|" + translate + "|" + scale + "|" + rotate + "|" + skewX + "|" + skewY + ")", transforms = "(?:" + transform + "(?:" + commaWsp + transform + ")*" + ")", transformList = "^\\s*(?:" + transforms + "?)\\s*$", reTransformList = new RegExp(transformList), reTransform = new RegExp(transform, "g");
    ...
    fabric.parseSVGDocument = function() {
        var reAllowedSVGTagNames = /^(path|circle|polygon|polyline|ellipse|rect|line|image|text)$/, reNum = "(?:[-+]?\\d+(?:\\.\\d+)?(?:e[-+]?\\d+)?)", reViewBoxAttrValue = new RegExp("^" + "\\s*(" + reNum + "+)\\s*,?" + "\\s*(" + reNum + "+)\\s*,?" + "\\s*(" + reNum + "+)\\s*,?" + "\\s*(" + reNum + "+)\\s*" + "$");
        ...
        parseAttributes: function(element, attributes) {
            ...
            if (element.parentNode && /^(svg|g)$/i.test(element.parentNode.nodeName)) {
                ...
                if (element.parentNode.tagName == "svg") {
                    var isLeftSet = "left" in parentAttributes, isTopSet = "top" in parentAttributes;
...
(function() {
    ...
    fabric.Canvas = fabric.util.createClass(fabric.StaticCanvas, {
        ...
        _normalizePointer: function(object, pointer) {
            var activeGroup = this.getActiveGroup(), x = pointer.x, y = pointer.y, isObjectInGroup = activeGroup && object.type !== "group" && activeGroup.contains(object);
            if (isObjectInGroup) {
                x -= activeGroup.left;
                y -= activeGroup.top;
            }
        ...
        _setupCurrentTransform: function(e, target) {
            if (!target) return;
            var pointer = this.getPointer(e), corner = target._findTargetCorner(pointer), action = this._getActionFromCorner(target, corner), origin = this._getOriginFromCorner(target, corner);
        ...
        _isLastRenderedObject: function(e) {
            return this.controlsAboveOverlay && this.lastRenderedObjectWithControlsAboveOverlay && this.lastRenderedObjectWithControlsAboveOverlay.visible && this.containsPoint(e, this.lastRenderedObjectWithControlsAboveOverlay) && this.lastRenderedObjectWithControlsAboveOverlay._findTargetCorner(this.getPointer(e));
        },
        ...
        getPointer: function(e) {
            var pointer = getPointer(e, this.upperCanvasEl), bounds = this.upperCanvasEl.getBoundingClientRect(), cssScale;
    ...
    fabric.util.object.extend(fabric.Canvas.prototype, {
        ...
        _setCursorFromEvent: function(e, target) {
            ...
            if (!target || !target.selectable) {
                ...
                var activeGroup = this.getActiveGroup(), corner = target._findTargetCorner && (!activeGroup || !activeGroup.contains(target)) && target._findTargetCorner(this.getPointer(e));
                if (!corner) {
...
fabric.util.object.extend(fabric.Object.prototype, {
    ...
    getSvgTransform: function() {
        var toFixed = fabric.util.toFixed, angle = this.getAngle(), center = this.getCenterPoint(), NUM_FRACTION_DIGITS = fabric.Object.NUM_FRACTION_DIGITS, translatePart = "translate(" + toFixed(center.x, NUM_FRACTION_DIGITS) + " " + toFixed(center.y, NUM_FRACTION_DIGITS) + ")", anglePart = angle !== 0 ? " rotate(" + toFixed(angle, NUM_FRACTION_DIGITS) + ")" : "", scalePart = this.scaleX === 1 && this.scaleY === 1 ? "" : " scale(" + toFixed(this.scaleX, NUM_FRACTION_DIGITS) + " " + toFixed(this.scaleY, NUM_FRACTION_DIGITS) + ")", flipXPart = this.flipX ? "matrix(-1 0 0 1 0 0) " : "", flipYPart = this.flipY ? "matrix(1 0 0 -1 0 0)" : "";
...
(function() {
    var degreesToRadians = fabric.util.degreesToRadians, isVML = typeof G_vmlCanvasManager !== "undefined";
    fabric.util.object.extend(fabric.Object.prototype, {
        ...
        drawControls: function(ctx) {
            ...
            var size = this.cornerSize, size2 = size / 2, strokeWidth2 = ~~(this.strokeWidth / 2), left = -(this.width / 2), top = -(this.height / 2), paddingX = this.padding / this.scaleX, paddingY = this.padding / this.scaleY, scaleOffsetY = size2 / this.scaleY, scaleOffsetX = size2 / this.scaleX, scaleOffsetSizeX = (size2 - size) / this.scaleX, scaleOffsetSizeY = (size2 - size) / this.scaleY, height = this.height, width = this.width, methodName = this.transparentCorners ? "strokeRect" : "fillRect";
            ctx.save();
    ...
    fabric.Line = fabric.util.createClass(fabric.Object, {
        ...
        _render: function(ctx) {
            ...
            if (isInPathGroup && !this.transformMatrix) {
                ...
                ctx.translate(-this.group.width / 2 + cp.x, -this.group.height / 2 + cp.y);
            }
...
(function(global) {
    ...
    fabric.Rect = fabric.util.createClass(fabric.Object, {
        ...
        _render: function(ctx) {
            ...
            var rx = this.rx || 0, ry = this.ry || 0, w = this.width, h = this.height, x = -w / 2, y = -h / 2, isInPathGroup = this.group && this.group.type === "path-group", isRounded = rx !== 0 || ry !== 0;
            ctx.beginPath();
...
(function(global) {
    ...
    fabric.Path = fabric.util.createClass(fabric.Object, {
        ...
        _render: function(ctx) {
            var current, previous = null, x = 0, y = 0, controlX = 0, controlY = 0, tempX, tempY, tempControlX, tempControlY, l = -(this.width / 2 + this.pathOffset.x), t = -(this.height / 2 + this.pathOffset.y), methodName;
...
(function(global) {
    ...
    fabric.PathGroup = fabric.util.createClass(fabric.Path, {
        ...
        toSVG: function(reviver) {
            var objects = this.getObjects(), markup = [ "<g ", 'style="', this.getSvgStyles(), '" ', 'transform="', this.getSvgTransform(), '" ', ">" ];
            for (var i = 0, len = objects.length; i < len; i++) {
                markup.push(objects[i].toSVG(reviver));
            }
            markup.push("</g>");
...
(function(global) {
    ...
    fabric.Group = fabric.util.createClass(fabric.Object, fabric.Collection, {
        ...
        _getBounds: function(aX, aY, onlyWidthHeight) {
            var minX = min(aX), maxX = max(aX), minY = min(aY), maxY = max(aY), width = maxX - minX || 0, height = maxY - minY || 0, obj = {
                width: width,
                height: height
            };
            if (!onlyWidthHeight) {
...
(function(global) {
    ...
    fabric.Image = fabric.util.createClass(fabric.Object, {
        ...
        toSVG: function(reviver) {
            var markup = [];
            markup.push('<g transform="', this.getSvgTransform(), '">', '<image xlink:href="', this.getSvgSrc(), '" style="', this.getSvgStyles(), '" transform="translate(' + -this.width / 2 + " " + -this.height / 2 + ")", '" width="', this.width, '" height="', this.height, '" preserveAspectRatio="none"', "></image>");
            if (this.stroke || this.strokeDashArray) {
        ...
        _render: function(ctx) {
            this._element && ctx.drawImage(this._element, -this.width / 2, -this.height / 2, this.width, this.height);
        },
...
(function() {
    ...
    fabric.IText = fabric.util.createClass(fabric.Text, fabric.Observable, {
        ...
        _getCursorBoundaries: function(chars, typeOfBoundaries) {
            var cursorLocation = this.get2DCursorLocation(), textLines = this.text.split(this._reNewline), left = Math.round(this._getLeftOffset()), top = -this.height / 2, offsets = this._getCursorBoundariesOffsets(chars, typeOfBoundaries, cursorLocation, textLines);
            return {
                left: left,
                top: top,
    ...
    getSelectionStartFromPointer: function(e) {
        ...
        for (var i = 0, len = textLines.length; i < len; i++) {
            ...
            if (mouseOffset.y < height) {
                return this._getNewSelectionStartFromOffset(mouseOffset, prevWidth, width, charIndex + i, jlen, j);
            }
        }
